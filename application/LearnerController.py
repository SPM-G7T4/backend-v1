...
class LearnerController():
            ...
            return jsonify(
                {
                    "code": 200,
                    "data": {
                        "learners": [learner.get_details() for learner in learnerList]
                    }
                }
            ), 200
    ...
    def get_eligibility_and_reason(self, input_learner_email, input_course_id, input_class_id):
        eligibility = True
        reason = None
        ...
        # Check Status
        dbEnrolmentStatus = Enrolment.query.with_entities(Enrolment.status).filter_by(learner_email = input_learner_email, course_id = input_course_id, class_id = input_class_id).first()[0]
        if dbEnrolmentStatus == "enrolled":
            eligibility = False
            reason = "Enrolled"
            return [eligibility, reason]

        elif dbEnrolmentStatus == "pending":
            eligibility = False
            reason = "Requested"
            return [eligibility, reason]
