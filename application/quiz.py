...
def create_quiz(request_body):
    try:
        lastQuiz = Quiz.query.order_by(-Quiz.quiz_id).first().json()
...
def view_quiz_details(quiz_id):
    try:
        ...
        # Get Quiz Name 
        quiz = Quiz.query.filter_by(quiz_id=quiz_id).first().json()
        ...
        for DB_question_obj in DB_questions_list:
            DB_question_diction = DB_question_obj.json()
            ...
            for DB_option_obj in DB_options_list:
                DB_option_diction = DB_option_obj.json()
